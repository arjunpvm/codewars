...
def type_of_function(domain, codomain, relations):
    ...
    if sorted(list(domain)) != sorted(rel_domain):
        return "It is not a function"

    elif sorted(list(set(rel_codomain))) == sorted(rel_codomain) and codomain == set(rel_codomain):
        return "Bijective"

    elif sorted(list(set(rel_codomain))) == sorted(rel_codomain):
        return "Injective"
    elif codomain == set(rel_codomain):
        return "Surjective"
    else:
